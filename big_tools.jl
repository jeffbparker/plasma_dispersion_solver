...
This module provides a path for doing these computations in arbitrary precision arithmetic.
In general ArbNumerics is used, but some packages are not compatible with it, and so for
some purposes we need to cast to BigFloat first.
"""
module tools
...
function numerical_complex_derivatve(f, a, h)
...
function theta_circle(N)
    """Return N points theta, uniformly distributed on the circle in [0, 2pi). Using ArbNumerics."""
    arb2pi = 2 * an.ArbReal(pi)
    theta = zeros(an.ArbReal, N)
    for j in 1:N
        theta[j] = arb2pi * (an.ArbReal(j)-1) / an.ArbReal(N)
    end
    return theta
end

function complex_arb_to_bigfloat_array(arbarray)
    """cast Arb Array to BigFloat array"""
    bigcomplex = BigFloat(1) + im*BigFloat("2")
    bfarray = zeros(typeof(bigcomplex), length(arbarray))
    for j in 1:length(arbarray)
        bfarray[j] = BigFloat(real(arbarray[j])) + im * BigFloat(imag(arbarray[j]))
    end
    return bfarray
end
...
function sq(theta, params::Sqparams)
    ...
    Inputs:
        theta      angle on unit circle (scalar an.ArbReal)
        params
    Outputs:
        xa         x-coordinate of base (unshifted) squircle, (scalar an.ArbReal)
        ya         y-coordinate of base (unshifted) squircle, (scalar an.ArbReal)
    """
    ...
    prefactor = sign(c*s) / sqrt(an.ArbReal(2))
...
function dsq_dtheta(theta, params::Sqparams)
    ...
    Inputs:
        ...
        params
    Outputs:
        dxa_dtheta  x-coordinate of derivative of base (unshifted) squircle, (scalar an.ArbReal)
        dya_dtheta  y-coordinate of derivative of base (unshifted) squircle, (scalar an.ArbReal)
    """
    a, smallnum = params.a, params.smallnum
...
function xyprime(theta, params::Sqparams)
    ...
    Inputs:
        ...
        params
    Outputs:
        xp         x-coordinate of shifted squircle, (scalar an.ArbReal)
        yp         y-coordinate of shifted squircle, (scalar an.ArbReal)
    """
...
function dxyprime_dtheta(theta, params::Sqparams)
    ...
    Inputs:
        ...
        params
    Outputs:
        dxp_dtheta      x-coordinate of shifted squircle, (scalar an.ArbReal)
        dyp_dtheta      y-coordinate of shifted squircle, (scalar an.ArbReal)
    """
    a, Rx, Ry, rx, epsilon = params.a, params.Rx, params.Ry, params.rx, params.epsilon
...
function gamma(theta, params::Sqparams)
    ...
    Inputs:
        ...
        params
    Outputs:
        z          coordinate of shifted squircle in complex form x + iy, (scalar an.ArbComplex)
    """
...
function dgamma_dtheta(theta, params::Sqparams)
    ...
    Inputs:
        ...
        params
    Outputs:
        dz_dtheta   derivative of shifted squircle in complex form, (scalar an.ArbComplex)
    """
...
function count_zeros_from_vals(vals)
    ...
    """
    vals = Vector{ComplexF64}(vals)  # cast to ComplexF64 values 
...
function count_zeros_inside_sq_contour(fun, N, params::Sqparams)
    ...
    Inputs:
        fun             complex analytic function within squircle contour f(z), takes an.ArbComplex
    ...
    for j = 1:N
        ...
        vals[j] = fun(point_on_squircle)  # implicit cast from ArbComplex to ComplexF64
    end
...
function integrate_fun_on_squircle(fun, N, params::Sqparams)
    ...
    Inputs:
        ...
        params          squircle params
    Outputs:
        result          approximation to value of integral of fun on the squircle contour (scalar an.ArbCompex)
    """
    theta = theta_circle(N)
    dtheta = (2 * an.ArbReal(pi)) / N
    
    #fout = Array{an.ArbComplex}(undef, N)
    fout = zeros(an.ArbComplex, N)
    for j in 1:N
...
function compute_Sarray(f, fprime, num_zeros, Npts, params::Sqparams)
    ...
    Inputs:
        f             complex analytic function f(z) for which to find zeros, takes an.ArbComplex
        fprime        complex function f'(z) = df/dz, takes an.ArbComplex
        ...
        params        squircle params
    Outputs:
        Sn            approximation to value of integrals Sn on the squircle contour (array an.ArbCompex)    
    """
    theta = theta_circle(Npts)
    Sn = zeros(an.ArbComplex, num_zeros)    
    for j in 1:Npts
...
function compute_Sarray(f_fprime, num_zeros, Npts, params::Sqparams)
    ...
    Inputs:
        f_fprime      function of z that computes and returns both f(z), f'(z).  Type of z should be an.ArbComplex
        num_zeros     number of zeros of f within the squircle contour (integer)
        Npts          Number of points to use on squircle contour for integration  (integer)
        params        squircle params
    Outputs:
...
function Sarray_to_Aarray(Sarray)
    ...
    Inputs:
        Sarray =     [S1, S2, ..., SN]     (array an.ArbComplex)
    
    Outputs:
        Aarray = [A0, A1, A2, ..., AN]     (array an.ArbComplex)
    """
    num_zeros = length(Sarray)
    Aarray = zeros(an.ArbComplex, num_zeros + 1)
    Aarray[1] = 1   # A0 = 1
...
function roots_of_poly(coeffarray)
    ...
    This package doesn't seem to be compatible with ArbNumerics; cast inputs to BigFloat Copmlex.

    Inputs:
        coeffarray   polynomial coefficients in usual order---lowest degree first. (array an.ArbComplex)
    Outputs:
        roots        roots of polynomial (array Complex BigFloat)
    """
    coeffarray2 = complex_arb_to_bigfloat_array(coeffarray)
    p = Polynomials.Polynomial(coeffarray2)
...
function root_with_largest_imag_part(roots)
    ...
    Inputs:
        roots        roots of polynomial. (array Complex BigFloat)
    Outputs:
        root         root of polynomial with largest imaginary part (scalar Complex BigFloat)
    """
...
function max_growth_rate(f, fprime, num_zeros, Npts, params::Sqparams)
    ...
    Inputs:
        ...
        params        squircle params
    Outputs:
        omega_r       Real part of eigenmode (real frequency)  (scalar an.ArbReal)
        omega_i       Imaginary part of eigenmode (growth rate)  (scalar an.ArbReal)
    """
    ...
    omega_r = an.ArbReal(real(root))
    omega_i = an.ArbReal(imag(root))
